...
path_name='/udd/hzhang/SmoothAdversarialExamples/dataset/zibra/';
pic_list=dir(path_name);
p_n=size(pic_list);
for i=3:p_n
    name=pic_list(i).name;
%     name = '641d03527848417f.png';
...
%         Aa=speye(size(S))-(alpha/(1-alpha))*S;
        A= graph_matrix(Aa,m,image_index,299);
        HA(:,:,dim_i)=A';
    end
    A=reshape(HA,[4,299,299,3]);
    data_str=sprintf('/nfs/nas4/data-hanwei/data-hanwei/DATA/SmoothPerturbation/imagenet/A1/SpA_%s_%f_%f.mat',name,namuda,alpha);
%     data_str=sprintf('/nfs/pyrex/raid6/hzhang/2017-nips/test_A/SpA_%s_%f_%f.mat',name,namuda,alpha);
    save(data_str,'A');
end
end
