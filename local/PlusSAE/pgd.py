...
class PGD_tf:
    ...
    def __init__(self, model, batch_shape,
                 steps, targeted, gamma = 0.05,norm = 2
        ...
        self.init = tf.variables_initializer(var_list=[delta, self.best_l2, self.best_delta, adv_found])
        ...
        # self.lr small step size
        new_delta = delta - self.lr * grad_without_zeros
        # new step size
        new_l2 = tf.norm(tf.layers.flatten(new_delta), axis=1)
        #new_norm should be the global step size
        normer = tf.reshape(norm / new_l2, (-1, 1, 1, 1))
