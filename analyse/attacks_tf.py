...
class CarliniWagnerL2(object):
  ...
  def attack(self, imgs, targets):
    ...
    for i in range(0, len(imgs), self.batch_size):
      ...
      a,b = self.attack_batch(imgs[i:i + self.batch_size],
  ...
  def attack_batch(self, imgs, labs):
    ...
    num_iter = np.zeros((1,self.BINARY_SEARCH_STEPS))
...
class Clip_version_debug(object):
    ...
    def __init__(self, sess, model, batch_size, confidence,
        ...
        #modifier = tf.Variable(tf.random_normal((A_shape)))
      #  modifier = tf.reshape(modifier,(None,784,1))
        # these are variables to be more efficient in sending data to tf
        self.A = tf.Variable(
        ...
        def body(i, r, p, s, x):
            q = multi_sparse(p, self.A, batch_size, shape)
        ...
        def f_false(modifier):
            i = tf.constant(0)
            r = modifier - multi_sparse(modifier, self.A, batch_size, shape)
            p = r
            ...
            i = tf.constant(0)
            r = z_d - multi_sparse(z_d, self.A, batch_size, shape)
            p = r
            s = tf.matmul(tf.transpose(r, [0, 2, 1]), r)
            x = z_d
