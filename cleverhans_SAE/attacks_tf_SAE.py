...
class SmoothCarliniWagnerL2CG(object):
...
class Clip_version_eig_norm(object):
    ...
    def __init__(self, sess, model, batch_size, confidence,
                 ...
                 binary_search_steps, max_iterations,
                 abort_early, initial_const,
                 clip_min, clip_max, eig_num, num_labels, shape):
        """
        Return a tensor that constructs adversarial examples for the given
        input. Generate uses tf.py_func in order to operate over tensors.
        :param sess: a TF session.
        ...
        # the variable we're going to optimize over
        modifier = tf.Variable(
            np.zeros((batch_size, shape[3], self.eig_num), dtype=np.float32))
        ...
        self.const = tf.Variable(np.zeros(batch_size), dtype=tf.float32,
                                 name='const')
        self.A = tf.Variable(np.zeros(
            (batch_size, self.eig_num, shape[2]*shape[1])), dtype=tf.float32, name='A')
        self.At = tf.Variable(np.zeros(
            (batch_size, self.eig_num, self.eig_num)), dtype=tf.float32, name='At')
        ...
        self.assign_const = tf.placeholder(tf.float32, [batch_size],
                                           name='assign_const')
        self.assign_A = tf.placeholder(tf.float32, (batch_size, self.eig_num, shape[2]*shape[1]),
                                       name='assign_A')
        self.assign_At = tf.placeholder(tf.float32, (batch_size, self.eig_num, self.eig_num),
                                        name='assign_At')

        smo_mod = tf.matmul(modifier, self.At)
        smo_mod = tf.matmul(smo_mod, self.A)
        ...
        self.setup.append(self.const.assign(self.assign_const))
        self.setup.append(self.A.assign(self.assign_A))
        self.setup.append(self.At.assign(self.assign_At))

        self.init = tf.variables_initializer(var_list=[modifier] + new_vars)

    def attack(self, imgs, targets, As, Ats):
        """
        Perform the L_2 attack on the given instance for the given targets.
        If self.targeted is true, then the targets represents the target labels
        If self.targeted is false, then targets are the original class labels
        ...
        for i in range(0, len(imgs), self.batch_size):
            ...
            r.extend(self.attack_batch(imgs[i:i + self.batch_size],
                                       targets[i:i + self.batch_size],
                                       As[i:i + self.batch_size],
                                       Ats[i:i + self.batch_size]))
        return np.array(r)

    def attack_batch(self, imgs, labs, As, Ats):
        """
        Run the attack on a batch of instance and labels.
        """
        def compare(x, y):
